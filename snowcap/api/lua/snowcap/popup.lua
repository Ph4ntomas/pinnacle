...
---@field layer? snowcap.layer.LayerHandle Parent handle: Layer.
---@field popup? snowcap.popup.PopupHandle Parent handle: Popup.
...
---@field no_grab? boolean If true, the Popup will no request an explicit keyboard grab upon creation.
...
function popup.new_widget(args)
    ...
    if args.layer then
        request.layer_id = args.layer.id
    elseif args.popup then
        request.popup_id = args.popup.id
    else
        ...
        for _, event in ipairs(response.widget_events) do
            ...
            elseif event.mouse_area then
                if callbacks[widget_id] ~= nil then
                    msg = widget._mouse_area_process_event(callbacks[widget_id], event.mouse_area --[[@as snowcap.widget.mouse_area.Event]])
                end
            elseif event.text_input then
                if callbacks[widget_id] ~= nil then
                    msg = widget._text_input_process_event(callbacks[widget_id], event.text_input --[[@as snowcap.widget.text_input.Event]])
                end
            end
